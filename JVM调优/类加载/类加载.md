...
[![img](a611bd2cf12204bee0a35159a9c3e95b100x100.webp)](https://juejin.cn/user/4406498334093895)

[变速风声![lv-4](lv-4.a78c420.png)](https://juejin.cn/user/4406498334093895)

2022年05月27日 09:20 · 阅读 11086

## 参考资料

- 《深入理解Java虚拟机》周志明 - 第7章 虚拟机类加载机制
- [JVM类加载机制 | 陈树义Blog](https://link.juejin.cn/?target=https%3A%2F%2Fwww.cnblogs.com%2Fchanshuyi%2Fp%2Fjvm_serial_07_jvm_class_loader_mechanism.html)
- [Java类加载器classloader的原理及应用 | 掘金](https://juejin.cn/post/6931972267609948167)
- [你确定你真的理解"双亲委派"了吗 | Blog](https://link.juejin.cn/?target=https%3A%2F%2Fwww.cnblogs.com%2Fhollischuang%2Fp%2F14260801.html)
- [Java常用机制 - SPI机制详解 | Java全栈知识体系](https://link.juejin.cn/?target=https%3A%2F%2Fpdai.tech%2Fmd%2Fjava%2Fadvanced%2Fjava-advanced-spi.html)
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c55ebc4bf9924d2d87eb5dfdd2e976f5~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
1. `main()` 方法的 `new Son()` 语句将触发实例化对象，调用 `Son` 的构造函数，调用子类的构造函数时会先调用父类的构造函数。所以，程序会输出如下信息。
...
1. 如上代码所示，会先执行 `static Book book = new Book()`。这条语句又触发了类的实例化，所以会执行「对象初始化方法」，如下所示。
...
1. 需要注意的是，`part 1` 和 `part 2` 的先后顺序，是根据它们在代码中出现的顺序决定的。`part 3` 部分是构造函数部分，这部分永远是出现最后的，和它在代码中的顺序无关。在代码中，`part 3` 部分虽然出现在 `part 2` 的前面，但在「对象初始化方法」中，它永远是出现在最后的。
2. 此外，由于 `part 2` 出现在 `part 3` 前面，所以输出 `price` 的值是 110，而不是 0。
3. 至此，程序输出如下。
...
1. 继续，执行完 `static Book book = new Book()` 语句后，回到步骤 2 中，执行「类初始化方法」。此时，程序会输出 `书的静态代码块`。
2. 继续，执行 `main()` 方法的 `staticFunction();` 语句。此时，程序会输出 `书的静态方法`。
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3899fdff662340559913f86253acdcb6~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
public abstract class ClassLoader {
    ...
    protected synchronized Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
        ...
        Class c = findLoadedClass(name);//检查是否已经被加载
        if (c == null) {
            try {
                //如果有父加载器，就从父加载器中加载，如果没有就从启动加载器中加载
                if (parent != null) {
...
- ref 1-[Java 9 中类加载器的变化 | Segmentfault](https://link.juejin.cn/?target=https%3A%2F%2Fsegmentfault.com%2Fa%2F1190000020847626)

![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/be979bbc08684a7191ba1e012a6ed08d~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
参考上述 Oracle 官网文档描述，Java 的类加载机制，更准确的说，应该叫做 “父委派模型”。但由于翻译问题，被称为了 “双亲委派机制”。参考 [Java类加载机制-双亲委派机制还是应该叫做“父委派模型” | CSDN](https://link.juejin.cn/?target=https%3A%2F%2Fblog.csdn.net%2Fu010841296%2Farticle%2Fdetails%2F89731566%3Futm_medium%3Ddistribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-3.control%26dist_request_id%3D6077030a-e458-4e66-badf-71ee51d25037%26depth_1-utm_source%3Ddistribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-3.control) 了解更多。
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f1943949581b4d93adb2a9d87066f78c~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
Java 提供了很多服务提供者接口（SPI，`Service Provider Interface`），它可以允许第三方为这些接口提供实现，比如数据库中的 SPI 服务 - JDBC。这些 SPI 的接口由 Java 核心类提供，实现者却是第三方。如果继续沿用双亲委派，就会存在问题，提供者由 Bootstrap ClassLoader 加载，而实现者是由第三方自定义类加载器加载。这个时候，顶层类加载就无法使用子类加载器加载过的类。
...
=====
    ...
    public Enumeration<URL> getResources(String name) throws IOException {
    	...
    	//tmp[0]中存放启动类加载器(Bootstrap ClassLoader)加载的类
        tmp[1] = findResources(name);//存放自定义类加载器加载的类
		//返回启动类加载的类和自定义加载器加载的类
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6fcf5fa655374cec8f5f1278a9d7a146~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2e785707db9e4ce599749f0995366015~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c8f6346517eb481080fa824bfa8c7800~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
1. 为每一个应用提供一个 `WebAppClassLoader` 加载器，负责加载应用自身目录下的 `class` 文件，从而**实现隔离**。
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/060ca146b8294d4d82b2af2e66f00b89~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)

结合下图，介绍下 Spring 官方推荐的热加载方案 —— Spring boot devtools。

![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/08aed0d0e88443e2a28cb0426ad059e1~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
        ...
        synchronized (getClassLoadingLock(name)) {
            ...
            if (c == null) {
                try {
                    ...
                    loadedClass = findClass(name);//从应用自身的目录加载
                } catch (ClassNotFoundException e) {
                    //如果没有加载到 则从父类加载
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/fb25ccf4402e4c0dae843d0ab9fafae4~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e0ba257cb4ad409996de955843af3194~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
- ref 1-[Java常用机制 - SPI机制详解 | Java全栈知识体系](https://link.juejin.cn/?target=https%3A%2F%2Fpdai.tech%2Fmd%2Fjava%2Fadvanced%2Fjava-advanced-spi.html)
- ref 2-[高级开发必须理解的Java中SPI机制](https://link.juejin.cn/?target=https%3A%2F%2Fwww.jianshu.com%2Fp%2F46b42f7f593c)
- ref 3-[SPI 机制是「可插拔」的奥义所在，SpringBoot Starter 也利用了这个特性](https://link.juejin.cn/?target=https%3A%2F%2Fmp.weixin.qq.com%2Fs%2FCGAzsC4wyrR68MOhZd0aLg)
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/48c771cc315b4ff6a1139f9b1da5396f~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
![java-spi-usage-1](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e18130294b524fb78685fa10964e9249~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)

下面给出一个具体的例子，展示 SPI 的使用。项目结构如下图所示。

![java-spi-usage-2](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/eac65d2b034241c5bf211f7f431c2b3a~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
...
- ref 1-[小议 SPI 和 API | Blog](https://link.juejin.cn/?target=https%3A%2F%2Fwww.cnblogs.com%2Fhappyframework%2Farchive%2F2013%2F09%2F17%2F3325560.html)
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/fd25b33fc55c4599864cbc776404c304~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image)
