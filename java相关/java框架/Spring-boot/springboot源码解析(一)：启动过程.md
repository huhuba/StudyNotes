...
public ConfigurableApplicationContext run(String... args) {
  ...
  //最终执行的方法：org.springframework.boot.context.event.EventPublishingRunListener#starting
  //广播应用正在启动事件
	listeners.starting();
	try {
        //5、初始化默认应用参数类
        ...
        //8、创建应用上下文，可以理解为创建一个容器：根据不同的应用类型，实例化不同的上下文
		context = createApplicationContext();
//9、准备异常报告器，用来支持报告关于启动的错误：从META-INF/spring.factories文件中读取、实例化
...
private ConfigurableEnvironment prepareEnvironment(SpringApplicationRunListeners listeners,
    ...
    //listeners环境准备，广播ApplicationEnvironmentPreparedEvent:环境已经准备好事件
...
8、创建应用的上下文:根据不同的应用类型初始化不同的上下文应用类
...
private void prepareContext(ConfigurableApplicationContext context, ConfigurableEnvironment environment,
		...
		listeners.contextLoaded(context);//发布上下文已经加载完毕事件
	}
-------------------
...
```java
refreshContext(context);//自动加载发生在这个方法里边，由此进入Spring的核心方法refresh()中。
    ...
    public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
                ...
                //激活各种beanfactory处理器：实例化并调用相关处理方法
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
//注册拦截bean创建的bean处理器，即注册beanPostProcessor：只是注册，没有执行调用，调用相关方法的位置
//在后边
...
17、发布应用上下文就绪事件：触发所有springapplicationRunnListener监听起的running事件方法
